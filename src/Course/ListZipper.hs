...
toList ::
  ListZipper a
  -> List a
toList (ListZipper xs y zs) = xs ++ (y :. Nil) ++ zs
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f z@(ListZipper xs y zs)
  | f y       = IsZ z
  | hasLeft z = findLeft f (ListZipper (tailPls xs) (headPls xs) (y :. zs))
  | otherwise = IsNotZ
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f z@(ListZipper xs y zs)
  | f y       = IsZ z
  | hasRight z = findRight f (ListZipper (y :. xs) (headPls zs) (tailPls zs))
  | otherwise = IsNotZ
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop z@(ListZipper Nil y zs) = undefined
