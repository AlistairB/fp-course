...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f (StateT fsa) = StateT $ \s -> first f <$> fsa s

-- HALP how does first work?
-- (\(a', s') -> (f a', s'))
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    -> StateT s f a
    -> StateT s f b
  (<*>) (StateT fsfab) (StateT fsa) = StateT $ \s -> let fa = fsfab s
...
instance Monad f => Monad (StateT s f) where
  (=<<) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (=<<) f (StateT fsa) = StateT $ \s -> let fa = fsa s
                                        in  fa >>= \(a, s') -> let (StateT fsb) = f a
                                                               in  fsb s'
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' (StateT sa) s = let (ExactlyOne r) = sa s
                          in  r
...
exec' ::
  ...
  -> s
  -> s
exec' (StateT sa) s = let (ExactlyOne r) = sa s
                      in  snd r
...
eval' ::
  ...
  -> s
  -> a
eval' (StateT sa) s = let (ExactlyOne r) = sa s
                      in  fst r
...
instance Monad f => Monad (OptionalT f) where
  (=<<) f (OptionalT fa) = OptionalT $ fa >>= \a -> case a of
...
instance Monad (Logger l) where
  (=<<) f (Logger xs a) = case f a of
                            (Logger xs' a') -> Logger (xs ++ xs') (a')
